...
class Employee extends Model
{
    ...
    protected static function boot()
    {
        ...
        static::addGlobalScope('user', function (Builder $builder) {
            if(auth()->check() && auth()->user()->type !=1){
                $builder->where('user_id', auth()->user()->id); //  if in header return $query->where('field', request()->field);
            }
        });
    }
    ...
    protected $fillable = [
        ...
        'country','company_id','user_id','facebook','skype','whatsapp','twitter','linkedin','hourly_rate','basic_salary'
    ];
