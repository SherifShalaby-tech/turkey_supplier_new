...
class TranslationServicesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $translation_services = TranslationServices::where('deleted_at', '=', null)
        ...
        $totalRows = $translation_services->count();
        $translation_services = $translation_services->offset($offSet)
            ->limit($perPage)
            ->orderBy($order, $dir)
            ->get();
    ...
    public function store(Request $request)
    {
        ...
        \DB::transaction(function () use ($request) {
            ...
            $TranslationServices = new TranslationServices;

            $TranslationServices->title = $request['title'];
            $TranslationServices->image = $filename;
     ...
     public function update(Request $request, $id)
     {
        ...
        // $this->authorizeForUser($request->user('api'), 'update', Brand::class);
         ...
         \DB::transaction(function () use ($request, $id) {
             ...
             $currentImage = $TranslationServices->image;

             if ($currentImage && $request->image != $currentImage) {
                 $image = $request->file('image');
                 $path = public_path() . '/images/translation_services';
                 $filename = rand(11111111, 99999999) . $image->getClientOriginalName();
                 ...
                 if (file_exists($TranslationServicesImage)) {
                     if ($currentImage != 'no-image.png') {
                         @unlink($TranslationServicesImage);
                     }
                 }
             ...
             TranslationServices::whereId($id)->update([
                 'title' => $request['title'],
                 'image' => $filename,
             ]);
    ...
    public function delete_by_selection(Request $request)
    {
      ...
      //  $this->authorizeForUser($request->user('api'), 'delete', Brand::class);
        ...
        foreach ($selectedIds as $brand_id) {
            TranslationServices::whereId($brand_id)->update([
                'deleted_at' => Carbon::now(),
            ]);
        }
        return response()->json(['success' => true]);
