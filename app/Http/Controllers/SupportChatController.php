...
class SupportChatController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $SupportChat = SupportChat::with('user')->where(function ($query) use ($request) {
                return $query->when($request->filled('search'), function ($query) use ($request) {
                    return $query->where('message', 'LIKE', "%{$request->search}%");
                });
            });
        $totalRows = $SupportChat->count();
        $SupportChat = $SupportChat->offset($offSet)
            ->limit($perPage)
            ->orderBy($order, $dir)
            ->get();
