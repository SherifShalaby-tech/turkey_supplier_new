...
class ShipmentOrdersController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $shipment_orders = ShipmentOrder::where(function ($query) use ($request) {
                return $query->when($request->filled('search'), function ($query) use ($request) {
                    return $query->where('name', 'LIKE', "%{$request->search}%")
                        ->orWhere('description', 'LIKE', "%{$request->search}%");
                });
            });
        $totalRows = $shipment_orders->count();
        $shipment_orders = $shipment_orders->offset($offSet)
            ->limit($perPage)
            ->orderBy($order, $dir)
            ->get();

            foreach ($shipment_orders as $order) {

                $item['id'] = $order['id'];
                $item['long_m'] = $order['long_m'];
                $item['long_c'] = $order['long_c'];
                $item['high_m'] = $order['high_m'];
                $item['high_c'] = $order['high_c'];
                $item['wide_c'] = $order['wide_c'];
                $item['wide_m'] = $order['wide_m'];
                $item['email'] = $order['email'];
                $item['full_name'] = $order['full_name'];
                $item['phone'] = $order['phone'];
                $item['shipment_type'] = $order['shipment_type'];
                $item['user_name'] = $order['user']['username'];
                $item['shipment_services'] = $order['shipment_services']['name'];
                $data[] = $item;
            }

        return response()->json([
            'shipment_orders' => $data,
            'totalRows' => $totalRows,
        ]);
