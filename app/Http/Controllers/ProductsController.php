...
class ProductsController extends BaseController
{
  ...
  // Search With Multiple Param
    ...
    public function store(Request $request)
    {
        ...
        try {
            $this->validate($request, [
                ...
                'cost' => 'required',
                'unit_id' => 'required',
                'OneToTwoUnit' => 'required',
                'GreaterEqualThreeUnit' => 'required',
                'Applicable_Industries' => 'required',
                'Place_Origin' => 'required',
                'Condition' => 'required',
                'Operating_Weight' => 'required',
                'Max_Digging_Height' => 'required',
                'Machine_Weight' => 'required',
                'Rated_Speed' => 'required',
                'Power' => 'required',
            ...
            \DB::transaction(function () use ($request) {
                ...
                $Product->OneToTwoUnit = $request['OneToTwoUnit'];
                $Product->GreaterEqualThreeUnit = $request['GreaterEqualThreeUnit'];
                $Product->Applicable_Industries = $request['Applicable_Industries'];
                $Product->Place_Origin = $request['Place_Origin'];
                $Product->Condition = $request['Condition'];
                $Product->Operating_Weight = $request['Operating_Weight'];
                $Product->Max_Digging_Height = $request['Max_Digging_Height'];
                $Product->Machine_Weight = $request['Machine_Weight'];
                $Product->Rated_Speed = $request['Rated_Speed'];
                ...
                $Product->user_id = $request['company_id'] ? $request['company_id'] : auth()->user()->id;
    ...
    public function create(Request $request)
    {
        ...
        $users = User::where('deleted_at', null)->where('role_id',2)->get(['id', 'username']);
        return response()->json([
            'categories' => $categories,
            'companies' => $users,
            'brands' => $brands,
