  ...
  <div class="main-content">
    ...
    <validation-observer ref="Add_payment">
      ...
      >
       ...
       <!-- Modal Change_Status-->
       <validation-observer ref="Change_Status">
        <b-modal
          hide-footer
          size="lg"
          id="Change_Status"
        ...
        >
          <b-form @submit.prevent="Submit_Status">
            <b-row>
...
export default {
  ...
  methods: {
    ...
    Submit_Payment() {
      ...
      //------ Validate Form Submit_Payment
      Submit_Status() {
      this.$refs.Change_Status.validate().then(success => {
        if (!success) {
          return;
        } else {
            this.Update_Status();
    ...
    Update_Status() {
      ...
      axios
        .post("changeStatus/" + self.sale.id, self.sale)
        .then(response => {
           self.SubmitProcessing = false;
          Fire.$emit("Event_Status");
  ...
  created() {
    ...
    Fire.$on("Event_Status", () => {
      setTimeout(() => {
        this.Get_Sales(this.serverParams.page);
        this.$bvModal.hide("Change_Status");
      }, 500);
    });
