<template>
  <div class="main-content">
    <breadcumb :page="$t('TranslationServices')" :folder="$t('Settings')"/>
        ...
        <template slot="table-row" slot-scope="props">
          ...
          <span v-else-if="props.column.field == 'image'">
            <b-img
              ...
              width="50"
              fluid
              :src="'/images/translation_services/' + props.row.image"
              alt="image"
            ></b-img>
          </span>
        </template>
        ...
        <b-form @submit.prevent="Submit_Brand" enctype="multipart/form-data">
          <b-row>
            ...
            <b-col md="12">
              ...
              >
                <b-form-group :label="$t('translationServicesTitle')">
                  <b-form-input
                    :placeholder="$t('tradeshowTitle')"
...
export default {
  ...
  computed: {
    columns() {
      return [
        {
          label: this.$t("TranslationServicesImage"),
          field: "image",
          tdClass: "text-left",
          thClass: "text-left"
        },
  ...
  methods: {
    ...
    Get_Brands(page) {
      ...
      axios
        ...
        .then(response => {
          this.translation_services = response.data.translation_services;
    ...
    Create_Brand() {
      ...
      self.data.append("title", self.ad.title);
      self.data.append("image", self.ad.image);
      axios
        .post("translation_services", self.data)
        .then(response => {
          ...
          this.makeToast(
            "success",
            this.$t("Create.TitleTranslationServices"),
            this.$t("Success")
          );
        })
        .catch(error => {
    ...
    Update_Brand() {
      ...
      axios
        .post("translation_services/" + self.ad.id, self.data)
        .then(response => {
           self.SubmitProcessing = false;
          Fire.$emit("Event_Brand");
          ...
          this.makeToast(
            "success",
            this.$t("Update.TitleTranslationServices"),
            this.$t("Success")
          );
        })
        .catch(error => {
    ...
    Delete_Brand(id) {
      this.$swal({
        ...
        if (result.value) {
          axios
            .delete("translation_services/" + id)
            .then(() => {
              this.$swal(
                this.$t("Delete.Deleted"),
                this.$t("Delete.TitleTranslationServices"),
                "success"
              );
    ...
    delete_by_selected() {
      this.$swal({
        ...
        if (result.value) {
          ...
          NProgress.set(0.1);
          axios
            .post("translation_services/delete/by_selection", {
