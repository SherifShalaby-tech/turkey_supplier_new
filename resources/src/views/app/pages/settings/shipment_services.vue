<template>
  <div class="main-content">
    <breadcumb :page="$t('ShipmentServices')" :folder="$t('Settings')"/>
        ...
        <template slot="table-row" slot-scope="props">
          ...
          <span v-else-if="props.column.field == 'image'">
            <b-img
              ...
              width="50"
              fluid
              :src="'/images/shipment_services/' + props.row.image"
              alt="image"
            ></b-img>
          </span>
        </template>
        ...
        <b-form @submit.prevent="Submit_Brand" enctype="multipart/form-data">
          <b-row>
            ...
            <b-col md="12">
              ...
              >
                <b-form-group :label="$t('AdsTitle')">
                  <b-form-input
                    :placeholder="$t('Enter_Ads_Title')"
             ...
             <b-col lg="12" md="12" sm="12">
               ...
               <!-- basic_shipping_service	 -->
             <b-col lg="12" md="12" sm="12">
                <validation-provider
                  name="basic_shipping_service"
                  :rules="{ required: true}"
                  v-slot="validationContext"
                >
                  <b-form-group :label="$t('basic_shipping_service')">
                     <textarea
                      :state="getValidationState(validationContext)"
                      aria-describedby="basic-shipping-service-feedback"
                      v-model="ad.basic_shipping_service"
                      class="form-control"
                      :placeholder="$t('basic_shipping_service')"
                     ></textarea>
                    <b-form-invalid-feedback
                      id="basic-shipping-service-feedback"
                    >{{ validationContext.errors[0] }}</b-form-invalid-feedback>
                  </b-form-group>
                </validation-provider>
              </b-col>
            ...
            <b-col md="12">
              <validation-provider name="Image" ref="Image" rules="mimes:image/*|size:200">
                <b-form-group slot-scope="{validate, valid, errors }" :label="$t('AdsImage')">
                  <input
  ...
  computed: {
    columns() {
      return [
        ...
        {
          label: this.$t("basic_shipping_service"),
          field: "basic_shipping_service",
          tdClass: "text-left",
          thClass: "text-left"
        },
  ...
  methods: {
    ...
    Get_Brands(page) {
      ...
      axios
        ...
        .then(response => {
          this.shipment_services = response.data.shipment_services;
    ...
    Create_Brand() {
      ...
      self.data.append("name", self.ad.name);
      self.data.append("detail", self.ad.detail);
      self.data.append("basic_shipping_service", self.ad.basic_shipping_service);
      self.data.append("image", self.ad.image);
      axios
        .post("shipment_services", self.data)
        .then(response => {
    ...
    Update_Brand() {
      ...
      axios
        .post("shipment_services/" + self.ad.id, self.data)
        .then(response => {
           self.SubmitProcessing = false;
          Fire.$emit("Event_Brand");
    ...
    Delete_Brand(id) {
      this.$swal({
        ...
        if (result.value) {
          axios
            .delete("shipment_services/" + id)
            .then(() => {
              this.$swal(
                this.$t("Delete.Deleted"),
                this.$t("Delete.TitleAds"),
                "success"
              );
    ...
    delete_by_selected() {
      this.$swal({
        ...
        if (result.value) {
          ...
          NProgress.set(0.1);
          axios
            .post("shipment_services/delete/by_selection", {
