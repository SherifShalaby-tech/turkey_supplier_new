@extends('layouts.website.master')
@section('title','Search')
@section('content')

<section class="bg0 p-t-20 p-b-10">
    <div class="container">
        
        <div class="p-b-10 separator">
            <div class="flex-center bg1 p-0  b-rt-lb-20">
                <h3 class="ltext-102 cl0 p-r-l-10 p-10-40">
                    {{trans('custom.search_result')}}
                </h3>
            </div>
        </div>

        @if($media->count() > 0)
            <div>
                <div class="separator-f">
                    <div class="flex-center bg1 p-0  b-rt-lb-20">
                        <h3 class="ltext-102 cl0 p-r-l-10 p-10-40">
                            {{trans('custom.products')}}
                        </h3>
                    </div>
                </div>
                <div class="row ">
                    @foreach($media as $key => $value)
                        <div class="col-sm-3 col-md-3 col-lg-3 col-6 p-lr-10-md m-tb-10">  
                            <div class="block2  bg-s2">
                                <div class="block2-pic hov-img0">
                                    <a href="{{route('product.details',['id' => $value->mediable->id, 'slug' => $value->mediable->slug])}}">
                                        @if($value->mediable->firstMedia)
                                            <img class="p-tb-10 p-lr-10  p-lr-5-md height-r-h-sr" 
                                                src="{{ asset('images/products/'.$value->mediable->firstMedia->file_name) }}" alt="{{ $value->mediable->name }}"
                                                style="max-width: 100%;  height: 250px; width: 100% !important;
                                                        background-size: cover;
                                                        background-repeat: no-repeat;
                                                        background-position: 50% 50%;
                                                        border-radius: 30px;">
                                        @else
                                            <img src="{{ asset('images/no-image.png') }}" alt="{{ $value->mediable->name }}"
                                                style="max-width: 100%;  height: 250px; width: 100% !important;
                                                        background-size: cover;
                                                        background-repeat: no-repeat;
                                                        background-position: 50% 50%;
                                                        border-radius: 30px;">
                                        @endif
                                    </a>

                                </div>

                                <div class="block2-txt">
                                     <div class=" flex-col-l p-r-l-5">
                                        <span
                                            class="card-title-name-home-cat p-r-l-5 text-sm mtext-1075 cl2 hov-cl1 trans-04 js-name-b2  w-full ">
                                            {{$value->mediable->company->name}}<br>
                                        </span>
                                        <span class="cl1 w-full p-r-l-5 p-tb-2 mtext-1075 f-s-s">
                                            @php
                                                $helpers = new \App\utils\helpers();
                                                $new_price = $helpers->get_price($value->mediable->price);
                                            @endphp
                                            {{$code}} {{$new_price}}
                                        </span>
                                    </div>  
                           
                                    <div class=" p-lr-0-md p-b-5">
                                        <div class="flex-product">
                                            <form  action="{{route('carts.add.product')}}" method="POST">
                                                @csrf
                                                @if(auth('company')->user())
                                                    <input type="hidden" name="user_id" value="{{auth('company')->user()->id}}">
                                                @endif
                                                <input type="hidden" name="product_id" value="{{$value->mediable->id}}">
                                                <button class="btn btn-cart hover-shadow cl0 bg1 f-s-s" type="submit">
                                                    {{trans('custom.add_to_cart')}} <i class="fas fa-shopping-cart"></i></button>
                                            </form>

                                           {{-- <form action="{{route('fav.add')}}" method="POST">
                                                @csrf
                                                @if(auth('company')->user())
                                                    <input type="hidden" name="company_id" value="{{auth('company')->user()->id}}">
                                                @endif
                                                <input type="hidden" name="product_id" value="{{$value->mediable->id}}">
                                                <button  class="three btn btn-contact btn-chat hover-shadow  bg2 f-s-s  {{$value->mediable->is_like ? 'cl1':'cl0'}}">
                                                <i   class="fa-solid fa-heart "></i></button>
                                            </form> --}}
       <button  class="three btn btn-contact btn-chat hover-shadow cl0 bg2 f-s-s addToFavourte {{$value->mediable->IsFavourite?'cl1':''}}" data-productid="{{$value->mediable->id}}" data-companyid="{{auth('company')->user()?auth('company')->user()->id:''}}">
                                                    <i class="fa-solid fa-heart " style="pointer-events:none;"></i></button>
                                        </div>
                                    </div>

                                </div>

                                </div>
                            </div>
                    @endforeach
                </div>
            </div>
       
       
        
        @else
            <div class="add-address" style="margin-bottom: 135px">
                <p class="text-danger alert alert-danger">{{trans('custom.products_not_found')}}
                </p>
            </div>
        @endif

    </div>
</section>



@stop