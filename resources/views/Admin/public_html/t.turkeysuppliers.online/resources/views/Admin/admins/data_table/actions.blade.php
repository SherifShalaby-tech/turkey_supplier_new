@if (auth()->user()->hasPermission('update_admins'))
    <a href="{{ route('admin.admins.edit',$admin->id) }}" class="btn btn-info btn-sm">   {{ trans('site.edit') }}</a>
@endif
...
@endif
    <form action="{{ route('admin.admins.destroy', $admin->id) }}" class="my-1 my-xl-0" method="post" style="display: inline-block;" autocomplete="off">
        @csrf
        @method('delete')
