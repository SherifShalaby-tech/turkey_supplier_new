    <a href="{{ route('admin.clients.edit',$company->id) }}" class="btn btn-info btn-sm" target="_blank">   {{ trans('site.edit') }}</a>
    {{-- <a href="{{ route('admin.users.index',$company->id) }}" class="btn btn-primary btn-sm">   {{ trans('site.show') }}</a> --}}
    <a href="{{ route('admin.clients.showdata',$company->id) }}" class="btn btn-success btn-sm">   {{ trans('site.show') }}</a>
    <button type="button" class="btn btn-btn btn-danger btn-sm " data-toggle="modal" data-target="#delete{{ $company->id }}">
        <i class="fa fa-trash"></i>
        {{ __('site.delete') }}
    </button>
    <form action="{{ route('admin.clients.destroy', $company->id) }}" class="my-1 my-xl-0" method="post" style="display: inline-block;" autocomplete="off">
        @csrf
        @method('delete')
